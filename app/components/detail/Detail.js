...
function Detail() {
  ...
  const detailGet = async () => {
    await axios.get(`/api/member?key=${objKey}`).then((res) => {
      setDetailItem(res.data);
    });
  };

  const detailPortPicGet = async () => {
    await axios.get(`/api/portPic?sKey=${objKey}`).then((res) => {
      setDetailProtPic(res.data);
    });
  };

  const likeLoad = async () => {
    await axios.get(`/api/detail?sKey=${sKey}&objKey=${objKey}`).then((res) => {
  ...
  const likeTest = async () => {
    await axios.get(`/api/detail?sKey=${sKey}&objKey=${objKey}`).then((res) => {
      if (res.data) {
        axios.post(`/api/detail`, { key: Date.now().toString(), sKey: String(sKey), objKey: String(objKey) }).then((res) => {
          setLikeCheck(res.data);
          detailGet();
        });
      } else {
        axios.delete(`/api/detail?sKey=${sKey}&objKey=${objKey}`).then((res) => {
          setLikeCheck(res.data);
          detailGet();
        });
      }
  ...
  const loginUserInfoGet = async () => {
    await axios.get(`/api/member?key=${sKey}`).then((res) => {
      setLoginUserInfo(res.data);
    });
  };

  const commentLoad = async () => {
    await axios.get(`/api/comment?objKey=${objKey}`).then((res) => {
      setComment(res.data);
    });
  };
  ...
  const commentSubmit = async (e) => {
    ...
    await axios.post(`/api/comment`, objData).then((res) => {
      // setComment(res.data)
      commentLoad();
    });
  };
  ...
  const commentDel = async (commentItem) => {
    ...
    if (sel) {
      if (sKey === commentItem.sKey) {
        await axios.delete(`/api/comment?key=${commentItem.key}`).then((res) => {
          // setComment(res.data)
          commentLoad();
        });
      } else {
    ...
    <section className={styles.detailSection}>
      <div className={styles.top}>
        <button onClick={() => router.push("/pages/list")}>
          <img src="../asset/detail/arrow-gray-icon.svg"></img>
        </button>
        <p>상세보기</p>
      </div>
        ...
        <div className={styles.introInfoBox}>
          <div className={styles.introInfo}>
            ...
            <p className={styles.like}>
              <span>{detailItem[0].like}</span>명이 찜했습니다.
            </p>
        ...
        <ul className={styles.designerInfoCon}>
          <li>
            ...
            <p className={styles.right}>
              {detailItem[0].dTime1} ~ {detailItem[0].dTime2}
            </p>
          </li>
          <li>
          ...
          <li>
            ...
            <p className={styles.right}>
              {detailItem[0].dNumber1} - {detailItem[0].dNumber2} - {detailItem[0].dNumber3}
            </p>
          </li>
        </ul>
      </div>
        ...
        <ul>
          {comment.map((commentItem, index) => (
            <li key={index}>
              <div className={styles.left}>
                {member
                  .filter((item) => item.key == commentItem.sKey)
                  .map((obj, index) => (
                    <img key={index} src={obj.imgUrl}></img>
                  ))}
              </div>
              <div className={styles.right}>
                <div className={styles.authorBox}>
                  ...
                  <div className={styles.right}>
                    <p onClick={() => commentDel(commentItem)} className={styles.delete}>
                      삭제하기
                    </p>
                  </div>
                </div>
